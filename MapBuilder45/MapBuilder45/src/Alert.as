package{
	...
	public class Alert {
		...
		private static var btnWidth:int = 120
		...
		import Alert;
							Alert.init(stage);
							//
							btnSim.addEventListener("click", showSimpleAlert);
							btnAdv.addEventListener("click", showAlertWithCustomButtons);
							btnOpt.addEventListener("click", showAlertWithCustomOptions);
							//
							Text.text = "Write Desired Text Here";
							//	THE SIMPLEST WAY TO DISPLAY AN ALERT
							function showSimpleAlert(event:MouseEvent):void {
								Alert.show(Text.text);
							}
							//	SHOW ALERT WITH CUSTOM BUTTONS
							function showAlertWithCustomButtons(event:MouseEvent):void {
								var buttons:Array = new Array();
								buttons.push(btn1.text);
								if (btn2.text != "") {
									buttons.push(btn2.text);
								}
								if (btn3.text != "") {
									buttons.push(btn3.text);
								}
								Alert.show(Text.text, {buttons:buttons, callback:showAlertResponse});
							}
							//	SHOW ALERT WITH CUSTOM PARAMETERS
							function showAlertWithCustomOptions(event:MouseEvent):void {
								var myPosition:Point;
								if (xPos.text != "" && yPos.text != "") myPosition = new Point(int(xPos.text), int(yPos.text));
								Alert.show(Text.text, {background:BackgroundTypeSelector.selectedItem.data, colour:ColourSelector.selectedColor, position:myPosition, promptAlpha:PromptAlphaSelector.value, textColour:TextColourSelector.selectedColor});
							}
							//
							function showAlertResponse(response:String):void {
								Alert.show("The reponse was '"+response+"'", {colour:0xAAAAAA});
							}
